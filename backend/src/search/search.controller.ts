...
export class SearchController {
  ...
  @Get("suggestions")
  @RouteTypeDecorator(RouteType.PUBLIC_READ)
  @ThrottleOverride("SEARCH") // 150 requests per minute
  @ApiOperation({
    summary: "Autocomplete suggestions",
    description:
      "Returns artist and track suggestions for partial query (min 2 characters).",
  })
  ...
  getSuggestions(
    ...
    artists: { type: string; id: string; title: string; subtitle?: string }[];
    tracks: { type: string; id: string; title: string; subtitle?: string }[];
  }> {
  ...
  @ApiQuery({
    ...
    required: false,
    enum: ["artist", "track"],
    description: "Limit to artist or track",
  })
  ...
  @ApiQuery({
    ...
    required: false,
    enum: ArtistStatus,
    description: "Artist status filter",
  })
  @ApiQuery({
    name: "country",
    required: false,
    description: "Artist country filter (ISO alpha-2)",
  })
  @ApiQuery({
    name: "city",
    required: false,
    description: "Artist city filter (partial, case-insensitive)",
  })
  @ApiQuery({
    name: "hasLocation",
    required: false,
    type: Boolean,
    description: "Only artists with public location",
  })
  @ApiQuery({
    name: "isVerified",
    required: false,
    type: Boolean,
    description: "Only verified artists",
  })
  @ApiQuery({
  ...
  @ApiResponse({
    status: 200,
    description: "Search results (artists and/or tracks, paginated)",
  })
